        ...
        if (dataRes.msg) {
          ...
          for (let i = 0; i < newNum; i++) {
            ...
            if (exited.current) {
              ...
              db.transaction(tx => {
                tx.executeSql(`INSERT OR IGNORE INTO ${batchTable} (batchid, acctno, prevreading, reading, volume, rate, acctname, capacity, brand, meterno, billdate, duedate, discdate, amount, classification, penalty, discount, acctgroup, fromdate, todate, location, reader, balance, note, printed) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`,
